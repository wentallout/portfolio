import { c as create_ssr_component, e as spread, f as escape_object, v as validate_component, d as add_attribute, b as subscribe, h as escape, g as each, j as null_to_empty } from "../../../../chunks/index3.js";
...
const MusicPlayer = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="${"player svelte-70ezuv"}"><audio autoplay="${"false"}"${add_attribute("onended", next, 0)}${add_attribute("src", "/ringtones/" + $musicList[currentSongIndex].audio, 0)}${add_attribute("currentTime", currentTime, 0)}${add_attribute("volume", volume, 0)}${add_attribute("this", audioEle, 0)}></audio>
	<div class="${"player svelte-70ezuv"}"><div class="${"current svelte-70ezuv"}"><div class="${"info svelte-70ezuv"}"><div class="${"info__name xxl-text svelte-70ezuv"}">${escape($musicList[currentSongIndex].name)}</div>

				<div class="${"song-time svelte-70ezuv"}"><div class="${"current-time xl-text svelte-70ezuv"}">${escape(format(currentTime))}</div>
					<div class="${"duration xl-text svelte-70ezuv"}">${escape(format(duration))}</div></div></div>
			<div class="${"controls svelte-70ezuv"}"><button class="${"mp-btn mp-btn-small svelte-70ezuv"}">${validate_component(Skip_back, "SkipBack").$$render(
    $$result,
    {
      width: "24",
      height: "24",
    ...
    {}
				...
				${playing ? `<button class="${"mp-btn svelte-70ezuv"}">${validate_component(Pause, "Pause").$$render(
    $$result,
    {
      width: "24",
      height: "24",
    ...
    {},
    {}
  )}</button>` : `<button class="${"mp-btn play-btn svelte-70ezuv"}">${validate_component(Play_fill, "PlayFill").$$render(
    $$result,
    {
      width: "24",
      height: "24",
    ...
    {}
				...
				<button class="${"mp-btn mp-btn-small svelte-70ezuv"}">${validate_component(Skip_forward, "SkipForward").$$render(
    $$result,
    {
      width: "24",
      height: "24",
    ...
    {}
				...
				<input type="${"range"}" id="${"volume-control"}" min="${"0"}" max="${"100"}"${add_attribute("value", volume * 100, 0)}${add_attribute("this", volumeEle, 0)}></div></div>

		<div class="${"position svelte-70ezuv"}"><input class="${"position__input svelte-70ezuv"}" type="${"range"}"${add_attribute("value", 0, 0)}${add_attribute("this", positionEle, 0)}></div>

		<div class="${"song-list svelte-70ezuv"}">${each($musicList, (music, i) => {
    return `<div class="${escape(null_to_empty(i == currentSongIndex ? "song-active" : "song"), true) + " svelte-70ezuv"}"><div class="${"song-name small-text svelte-70ezuv"}">${escape(music.name)}</div>
					<a class="${"download svelte-70ezuv"}"${add_attribute("href", "/ringtones/" + $musicList[i].audio, 0)}${add_attribute("download", $musicList[i].audio, 0)}>${validate_component(Download, "Download").$$render(
      $$result,
      {
        width: "16",
        height: "16",
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  return `<section>${validate_component(SectionTitle, "SectionTitle").$$render($$result, { sectionTitle: "Ringtone" }, {}, {
    default: ({ sectionIcon }) => {
      return `${validate_component(File_audio, "FileAudio").$$render($$result, Object.assign({}, sectionIcon), {}, {})}`;
    }
  })}
...
<section>${validate_component(SectionTitle, "SectionTitle").$$render($$result, { sectionTitle: "Spotify Playlist" }, {}, {
    default: ({ sectionIcon }) => {
