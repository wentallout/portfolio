import { c as create_ssr_component, d as add_attribute, v as validate_component, e as spread, f as escape_object, g as each, h as escape, j as null_to_empty, b as subscribe } from "../../chunks/index3.js";
import { L as Linkedin_logo, P as Phone, E as Envelope, I as Instagram_logo } from "../../chunks/instagram-logo.js";
...
const SocialButtons = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div${add_attribute("style", style, 0)} class="${"social-links svelte-9taofn"}">${validate_component(ExLink, "ExLink").$$render(
    $$result,
    {
...
const Cover = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="${"cover svelte-5kot30"}"><video class="${"video svelte-5kot30"}" autoplay muted loop${add_attribute("this", videoEle, 0)}><source src="${"/videos/city.mp4"}" type="${"video/mp4"}"></video>

	<a href="${"/"}" class="${"cover__logo svelte-5kot30"}">${validate_component(Yin_yang_fill, "YinYang").$$render(
    $$result,
    {
      class: "yinyang",
      width: "72",
	...
	<div class="${"cover__btn svelte-5kot30"}">${validate_component(SocialButtons, "SocialButtons").$$render($$result, { style: "flex-direction:column" }, {}, {})}</div>
</div>`;
});
...
<div class="${["bck2top svelte-1c25yb2", "hidden"].join(" ").trim()}">${validate_component(Caret_up, "CaretUp").$$render(
    $$result,
    {
      width: "24",
      height: "24",
...
<footer class="${"svelte-1t0qdhn"}"><div class="${"footer-container small-text svelte-1t0qdhn"}"><ul class="${"nav-list svelte-1t0qdhn"}">${each(navItems, (navItem) => {
    return `<li class="${"nav-list-item svelte-1t0qdhn"}"><a${add_attribute("href", navItem.path, 0)} class="${"svelte-1t0qdhn"}">${escape(navItem.title)}</a>
				</li>`;
  })}</ul>
...
${validate_component(VanishingHeader, "VanishingHeader").$$render(
    ...
    {
      default: () => {
        return `<header${add_attribute("this", headerEle, 0)}><nav class="${["nav svelte-1jvp8nn", ""].join(" ").trim()}"><ul class="${"nav-list svelte-1jvp8nn"}">${each(navItems, (navItem) => {
          return `<li class="${[
            "nav-list__item svelte-1jvp8nn",
            $page.url.pathname === `${navItem.path}` ? "active-page" : ""
          ].join(" ").trim()}"><a class="${"item svelte-1jvp8nn"}"${add_attribute("href", navItem.path, 0)}>${escape(navItem.title)}</a>
					</li>`;
        })}
...
<div class="${"overlay svelte-1jvp8nn"}"${add_attribute("this", navOverlayEle, 0)}><div class="${"closebtn svelte-1jvp8nn"}">${validate_component(X, "X").$$render(
    $$result,
    {
    ...
    return `<a class="${[
      ...
      $page.url.pathname === `${navItem.path}` ? "active-page" : ""
    ].join(" ").trim()}"${add_attribute("href", navItem.path, 0)}>${escape(navItem.title)}</a>`;
  })}
		<div class="${"overlay-item svelte-1jvp8nn"}">${validate_component(SocialButtons, "SocialButtons").$$render($$result, {}, {}, {})}</div></div>
</div>`;
});
...
const Breadcrumb = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<nav aria-label="${"breadcrumb"}" id="${"breadcrumb"}" class="${"breadcrumb base-text svelte-1q6c9w8"}">${each(crumbs, (c, i) => {
    return `${i == crumbs.length - 1 ? `${c.label !== "home" ? `<div class="${"breadcrumb__unclickable svelte-1q6c9w8"}">${escape(c.label)}
				</div>` : ``}` : `<a class="${"breadcrumb__clickable svelte-1q6c9w8"}"${add_attribute("href", c.href, 0)}>${escape(c.label)}</a> / `}`;
  })}
</nav>`;
});
const sanitize = "";
...
<div class="${"page-container svelte-1bbsrv2"}">${validate_component(BackToTop, "BackToTop").$$render($$result, {}, {}, {})}
	${validate_component(Header, "Header").$$render($$result, {}, {}, {})}

	<main class="${"main-content svelte-1bbsrv2"}" id="${"main-content"}">${validate_component(Breadcrumb, "Breadcrumb").$$render($$result, { path: $page.url.pathname }, {}, {})}
