...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	$$result.css.add(css);
	return `${validate_component(SectionTitle, 'SectionTitle').$$render(
		$$result,
		{ sectionTitle: 'Category' },
		{},
		{
			default: ({ sectionIcon }) => {
				return `${validate_component(Folder, 'Folder').$$render(
					$$result,
...
<div class="${'category svelte-1fxjn7a'}">${each(categories, (category) => {
		return `<a href="${'/blog/category/' + escape(category, true)}">${validate_component(
			Tag,
			'BlogTag'
		).$$render(
			$$result,
