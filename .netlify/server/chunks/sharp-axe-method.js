...
const You_tube = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  `,
					0
				)}><iframe${add_attribute('width', width, 0)}${add_attribute(
					'height',
					height,
					0
				)} class="${'youtube-sveltekit-embed'}"${add_attribute(
					'title',
					`youTube-${youTubeId}`,
					0
				)}${add_attribute(
					...
					src,
					0
				)} frameborder="${'0'}" allow="${'accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture'}" allowfullscreen${add_attribute(
					'style',
					`
      position: absolute;
      top: 0;
...
export { Sharp_axe_method as default, metadata };
