...
!(function (D, u) {
	...
	function emojiSafeSplit(D, u, F, C) {
		if (((D += ''), F && (D = D.trim ? D.trim() : D.replace(i, '')), u && '' !== u))
		...
		for (var E, e, t = [], n = D.length, B = 0; B < n; B++)
			((55296 <= (e = D.charAt(B)).charCodeAt(0) && e.charCodeAt(0) <= 56319) ||
				...
				((E = ((D.substr(B, 12).split(r) || [])[1] || '').length || 2),
				(e = D.substr(B, E)),
				(B += E - (t.emoji = 1))),
				t.push(
					'>' === e
	...
	var u,
		...
		F = {
			...
			init: function init(D, u, F) {
				...
				if (((s.svg = D.getBBox && ('TEXT' === A || 'TSPAN' === A)), !('innerHTML' in D || s.svg)))
					return !1;
				if (((s.target = D), 'value' in u)) {
					for (
						e = splitInnerHTML(D, f, !1, a),
							(g = g || document.createElement('div')).innerHTML = u.value,
							E = splitInnerHTML(g, f, !1, a),
							s.from = F._from,
							(!s.from && !p) || (p && s.from) || ((A = e), (e = E), (E = A)),
							s.hasClass = !(!l && !o),
							s.newClass = p ? o : l,
							s.oldClass = p ? l : o,
						...
						C.push(d);
					if ('diff' === u.type) {
						for (n = [], B = [], i = '', A = t = 0; A < E.length; A++)
							(r = E[A]) === e[A] ? (i += r) : ((n[t] = i + r), (B[t++] = i + e[A]), (i = ''));
					...
					t && D
						? ((C = B && a !== o),
							((F = n && a) ? "<span class='" + n + "'>" : '') +
								...
								s.slice(a).join(i) +
								(C ? '</span>' : ''))
						: e.slice(0, a).join(i) + i + s.slice(a).join(i)),
					u.svg
						? (r.textContent = E)
						: (r.innerHTML =
								'&nbsp;' === A && ~E.indexOf('  ') ? E.split('  ').join('&nbsp;&nbsp;') : E);
			}
		};
