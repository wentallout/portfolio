...
<form method="GET" class="flex gap-2 mt-4">
	...
	<!-- sync select value to query param via hidden input -->
			...
			{#each items as item (item.id)}
				<tr class="border-t">
					...
					<td class="p-3 text-right flex gap-2 justify-end">
						<a href={`/blogs/${item.slug}`} target="_blank" class="underline">View</a>
						<a href={`/studio/posts/${item.id}/edit`} class="underline">Edit</a>
